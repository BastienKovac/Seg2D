...
int main (int argc, char ** argv)
{
	...
	if (choice==1){
		config=Configuration(a_min,a_max,size_x,size_y,nb_ell,nb_ell_dont_accepted,im,d); // Version 1
	}
	if (choice==2){
		config=Configuration(a_min,a_max,size_x,size_y,nb_ell,nb_ell_dont_accepted,gradx,grady,M_PI/12,0.001,d); // Version 2
	}
	...
	try
	{
		...
		while(num_not_acc<stop){
			...
			if (i<nb_ell_config){
				...
				while ((i < nb_ell_tot) & (g->what_segment(i) == GraphType::SOURCE)){
					i++;
					if (i==nb_ell_tot) break;
				}
				if (i<nb_ell_tot){
					config=Configuration(new_config.get_Ellips(i-nb_ell_config),new_config.get_position(i-nb_ell_config),new_config.get_data_fit(i-nb_ell_config),nb_ell_tot);
					i++;
					accepted=true;
					for( ; i< nb_ell_tot ; i++){
						if(g->what_segment(i) == GraphType::SINK){
		...
		string file_name = "Segmentation_"+ name + ".txt";
		config.save_config(file_name);
		cout << "\nNumber of Ellipses found : " << config.get_nb_Ellipses() << endl;
		...
		cout << "\nCPU Time : " << floor(hours) << " hours " << floor(minuts) << " minuts " << second << " seconds " << endl;
